...
const LoadingSpinner = React.memo<{ size: 'sm' | 'md' | 'lg' }>(({ size }) => {
  return (
    <svg
      className={`animate-spin ${ICON_SIZE_CLASSES[size]} text-current`}
  ...
  ({
    ...
    const renderIcon = React.useCallback(
      (icon: React.ReactNode) => {
        if (!icon) return null
        return (
          <span className={`inline-flex ${ICON_SIZE_CLASSES[size]}`}>
            {icon}
          </span>
        )
      },
        ...
        return (
          <span
            className={`inline-flex items-center ${
              iconOnly ? 'justify-center' : GAP_CLASSES[size]
            }`}
          >
